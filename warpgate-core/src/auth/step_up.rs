...
mod tests {
    ...
    fn unit_is_fresh_clock_skew_future_stamp_is_fresh() {
        ...
        // skew, timezone corruption), don't force an extra Okta prompt.
    ...
    async fn unit_get_pubkey_last_sso_at_returns_none_when_never_stamped() {
        ...
        let pubkey_id = insert_user_and_pubkey(&*db.lock().await).await;

        let got = get_pubkey_last_sso_at(&*db.lock().await, pubkey_id)
            .await
            .unwrap();
        assert_eq!(got, None);
    }
    ...
    async fn unit_get_pubkey_last_sso_at_returns_none_for_missing_row() {
        ...
        let got = get_pubkey_last_sso_at(&*db.lock().await, missing)
            .await
            .unwrap();
        assert_eq!(got, None);
    }
    ...
    async fn unit_update_pubkey_last_sso_at_roundtrip() {
        ...
        let now = OffsetDateTime::now_utc();
        let rows_affected = update_pubkey_last_sso_at(&*db.lock().await, pubkey_id, now)
            .await
            .unwrap();
