...
async fn forward_web_auth_requests(
    ...
    loop {
        match state_change_rx.recv().await {
            ...
            Ok(_) => {}
            // The channel has capacity 1, so a burst of state changes drops
            // values. Losing an intermediate state is survivable; giving up is
            // not, because nothing else would ever surface a pending approval
            // for this auth state to the web UI.
